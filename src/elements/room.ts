...
export class RoomElement extends HTMLElement {
	...
	async join(session: Session, local: BroadcastPublisher): Promise<void> {
		...
		try {
			...
			if (this.#statusState.type !== "error") {
				this.#setStatus({
					type: "success",
					message: `✓ Joined room ${room.roomID} as ${local.name}`,
				});
			}
		} catch (e) {
			this.#setStatus({
				type: "error",
				message: `Failed to join: ${e instanceof Error ? e.message : String(e)}`,
			});
		}
	}
	...
	#setStatus(status: RoomLifecycleStatus): void {
		...
		this.dispatchEvent(
			new CustomEvent("statuschange", { detail: status, bubbles: true, composed: true }),
		);
	}

	#onJoin(member: JoinedMember): void {
		// Add participant to Light DOM (idempotent by name+type)
		const container = member.remote
		...
		// Dispatch join event
		this.dispatchEvent(
			new CustomEvent("join", { detail: member, bubbles: true, composed: true }),
		);
		...
		if (this.onjoin) {
			try {
				...
				this.#setStatus({
					type: "error",
					message: `onjoin handler failed: ${e instanceof Error ? e.message : String(e)}`,
				});
			}
		}
	}
	...
	#onLeave(member: LeftMember): void {
		...
		const selector = `[data-member-name="${member.name}"][data-member-type="${
			member.remote ? "remote" : "local"
		}"]`;
		...
		this.dispatchEvent(
			new CustomEvent("leave", { detail: member, bubbles: true, composed: true }),
		);
		...
		if (this.onleave) {
			try {
				...
				this.#setStatus({
					type: "error",
					message: `onleave handler failed: ${
