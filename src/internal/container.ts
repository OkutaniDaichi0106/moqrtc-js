import type { Frame } from "@okudai/moq";
import { varintLen, writeVarint } from "@okudai/moq/internal/mod.ts";
...
export class EncodedContainer implements Frame {
	...
	copyTo(dest: AllowSharedBufferSource): void {
		...
		if ((dest as { byteLength?: number }).byteLength === undefined) {
			...
			(dest as { byteLength: number }).byteLength <
				this.chunk.byteLength + varintLen(this.chunk.byteLength)
		) {
