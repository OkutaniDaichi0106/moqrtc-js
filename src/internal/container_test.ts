...
vi.mock("@okudai/moq/io", () => ({
	...
	readVarint: undefined /* TODO: Convert mock */.mockReturnValue([BigInt(0), 0]),
}));

import { varintLen, writeVarint } from "@okudai/moq/io";
...
describe("EncodedContainer", () => {
	...
	describe("constructor", () => {
		test("creates container with chunk", () => {
			...
			assertEquals(container.chunk, mockChunk);
		});
		...
		test("creates container with key chunk", () => {
			...
			assertEquals(container.chunk.type, "key");
		});
		...
		test("creates container with delta chunk", () => {
			...
			assertEquals(container.chunk.type, "delta");
		});
		...
		test("creates container with timestamp", () => {
			...
			assertEquals(container.chunk.timestamp, timestamp);
		});
	});
	...
	describe("byteLength", () => {
		test("returns chunk byte length", () => {
			...
			assertEquals(container.byteLength, 5);
		});
		...
		test("returns zero for empty chunk", () => {
			...
			assertEquals(container.byteLength, 0);
		});
		...
		test("returns correct length for large chunk", () => {
			...
			assertEquals(container.byteLength, 1000);
		});
	});
	...
	describe("copyTo", () => {
		...
		test("calls chunk.copyTo with correct subarray", () => {
			const sourceData = new Uint8Array([1, 2, 3]);
			const mockChunk = new MockEncodedChunk(sourceData);
			const copyToSpy = /* TODO: Convert spy */ undefined(mockChunk, "copyTo");
			const container = new EncodedContainer(mockChunk);
	...
	describe("Error Handling", () => {
		...
		test("handles chunk copyTo throwing error", () => {
			const mockChunk = new MockEncodedChunk(new Uint8Array([1, 2, 3]));
			/* TODO: Convert spy */ undefined(mockChunk, "copyTo").mockImplementation(() => {
	...
	describe("Boundary Value Tests", () => {
		test("handles maximum safe integer byte length", () => {
			...
			const mockChunk = {
				...
				copyTo: undefined, /* TODO: Convert mock */
			};
			const container = new EncodedContainer(mockChunk);

			assertEquals(container.byteLength, Number.MAX_SAFE_INTEGER);
		...
		test("handles single byte chunk", () => {
			...
			assertEquals(container.byteLength, 1);

			const dest = new Uint8Array(MEDIUM_BUFFER_SIZE);
			expect(() => container.copyTo(dest)).not.toThrow();

			// Verify the single byte is written correctly
			assertEquals(dest[1], 42);
		});
		...
		test("handles large chunk efficiently", () => {
			...
			assertEquals(container.byteLength, LARGE_CHUNK_SIZE);
		...
		test("handles chunk without timestamp", () => {
			...
			assertEquals(container.chunk.timestamp, undefined);
		});
		...
		test("handles chunk with zero timestamp", () => {
			...
			assertEquals(container.chunk.timestamp, 0);
			assertEquals(container.chunk.type, "key");
		});
...
describe("EncodedChunk Interface", () => {
	test("MockEncodedChunk implements interface correctly", () => {
		...
		assertEquals(chunk.type, "key");
		assertEquals(chunk.byteLength, 3);
		assertEquals(chunk.timestamp, 12345);
		assertEquals(typeof chunk.copyTo, "function");
	});
...
describe("Stress and Stability Tests", () => {
	test("handles multiple containers with different sizes", () => {
		...
		containers.forEach((container, i) => {
			assertEquals(container.byteLength, sizes[i]);
		});
	});
...
describe("Type Safety and Interface Compliance", () => {
	test("EncodedContainer implements Source interface", () => {
		...
		assertEquals(typeof container.byteLength, "number");
		assertEquals(typeof container.copyTo, "function");
	});
	...
	test("container maintains chunk reference integrity", () => {
		...
		assertEquals(container.chunk, chunk);
	...
	test("chunk type is preserved", () => {
		...
		assertEquals(keyContainer.chunk.type, "key");
		assertEquals(deltaContainer.chunk.type, "delta");
	});
	...
	test("timestamp is properly handled", () => {
		...
		assertEquals(container.chunk.timestamp, now);
		assertEquals(typeof container.chunk.timestamp, "number");
	});
});
...
describe("Edge Cases and Robustness", () => {
	test("handles chunk with undefined optional properties", () => {
		...
		assertEquals(container.chunk.timestamp, undefined);
		assertEquals(container.byteLength, 3);
	});
	...
	test("handles negative timestamp", () => {
		...
		assertEquals(container.chunk.timestamp, -1);
	});
	...
	test("handles fractional timestamp", () => {
		...
		assertEquals(container.chunk.timestamp, 123.45);
	});
	...
	test("container creation is idempotent", () => {
		...
		assertEquals(container1.chunk, container2.chunk);
		assertEquals(container1.byteLength, container2.byteLength);
	});
