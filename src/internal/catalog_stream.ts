...
import {
	CatalogLineSchema,
	TrackDescriptorSchema,
	TrackDescriptorsSchema,
...
import type { BytesFrame } from "@okutanidaichi/moqt";
				...
				while (true) {
					...
					if (isInit) {
						...
						if (this.version !== init.version) {
							err = new Error(
								`Catalog version mismatch: expected ${this.version}, got ${init.version}`,
							);
							break;
						}
