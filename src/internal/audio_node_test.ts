...
class MockAudioWorkletNode {
	...
	dispatchEvent = undefined /* TODO: Convert mock */.mockReturnValue(true);
}
...
describe('AudioNode', () => {
	...
	it('should disconnect all outputs', () => {
		...
		node1.connect(node2);
		node1.connect(node3);
		assertEquals(node1.outputs.size, 2);

		node1.disconnect();
		assertEquals(node1.outputs.size, 0);
		expect(node2.inputs.has(node1)).toBe(false);
...
describe('AudioEncodeNode', () => {
	...
	it('should return correct AudioNode properties', () => {
		...
		assertEquals(encodeNode.channelCountMode, "explicit");
...
describe('AudioDecodeNode', () => {
	...
	it('should return worklet properties', () => {
		...
		assertEquals(decoderNode.channelCountMode, "explicit");
