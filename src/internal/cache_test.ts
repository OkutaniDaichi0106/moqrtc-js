...
vi.mock("@okudai/moq", () => ({
	...
	TrackWriter: undefined, /* TODO: Convert mock */
}));
...
import type { Frame, GroupSequence, GroupWriter } from "@okudai/moq";
import { ExpiredGroupErrorCode, InternalGroupErrorCode } from "@okudai/moq";
import type { Source } from "@okudai/moq/io";
...
const createMockGroupWriter = () =>
	({
		...
		cancel: undefined, /* TODO: Convert mock */
...
const createMockFrame = () =>
	({
		...
		copyFrom: undefined, /* TODO: Convert mock */
	}) as any as Frame;
...
describe("GroupCache - Using Real Mutex/Cond", () => {
	...
	describe("constructor", () => {
		it("should create GroupCache with sequence and timestamp", () => {
			...
			assertEquals(cache.sequence, sequence);
			assertEquals(cache.timestamp, timestamp);
			assertEquals(cache.frames, []);
			assertEquals(cache.closed, false);
			assertEquals(cache.expired, false);
		});

		it("should handle zero sequence", () => {
			const cache = new GroupCache(0n, Date.now());
			assertEquals(cache.sequence, 0n);
		});
	...
	describe("append", () => {
		it("should append frame when cache is open", async () => {
			...
			assertEquals(cache.frames.length, 1);
			assertEquals(cache.frames[0], frame);
		});
		...
		it("should append source when cache is open", async () => {
			...
			assertEquals(cache.frames.length, 1);
			assertEquals(cache.frames[0], source);
		});
		...
		it("should not append when cache is closed", async () => {
			...
			assertEquals(cache.frames.length, 0);
		});
		...
		it("should append multiple frames in order", async () => {
			...
			assertEquals(cache.frames.length, 3);
			assertEquals(cache.frames[0], frame1);
			assertEquals(cache.frames[1], frame2);
			assertEquals(cache.frames[2], frame3);
		});
	...
	describe("close", () => {
		it("should close cache and broadcast via real Cond", async () => {
			...
			assertEquals(cache.closed, true);
		});
	...
	describe("expire", () => {
		it("should expire cache and clear frames", async () => {
			...
			assertEquals(cache.expired, true);
			assertEquals(cache.frames.length, 0);
		});
	...
	describe("State Management", () => {
		...
		it("should maintain frames array reference", async () => {
			...
			await cache.append(createMockFrame());
			assertEquals(cache.frames, originalRef);
...
describe("TrackCache Interface", () => {
	it("should have correct store method signature", () => {
		const mockTrackCache: TrackCache = {
			store: undefined, /* TODO: Convert mock */
...
describe("Integration Scenarios with Real Synchronization", () => {
	...
	it("should handle complex state transitions", async () => {
		...
		await cache.append(createMockFrame());
		await cache.append(createMockFrame());
		assertEquals(cache.frames.length, 2);
