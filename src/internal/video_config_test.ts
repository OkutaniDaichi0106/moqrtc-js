...
Deno.test("VideoConfig - Constants", async (t) => {
	await t.step("VIDEO_HARDWARE_CODECS contains expected codecs", () => {
	...
	await t.step("VIDEO_SOFTWARE_CODECS contains expected codecs", () => {
	...
	await t.step("codecs arrays are readonly at type level", () => {
...
Deno.test("VideoConfig - videoEncoderConfig", async (t) => {
	await t.step("calculates bitrate correctly for standard resolution", async () => {
	...
	await t.step("uses provided bitrate when specified", async () => {
		const options: VideoEncoderOptions = {
			width: 1280,
			height: 720,
			frameRate: 24,
	...
	await t.step("adjusts frameRate factor for different frame rates", async () => {
	...
	await t.step("uses hardware encoding when available and not Firefox", async () => {
		const options: VideoEncoderOptions = {
			width: 1280,
			height: 720,
			frameRate: 30,
		...
		// Note: Console debug calls are not mocked in Deno tests
	});

	await t.step("falls back to software encoding when hardware is disabled", async () => {
		const options: VideoEncoderOptions = {
			width: 1280,
			height: 720,
			frameRate: 30,
	...
	await t.step("skips Firefox warning test due to mocking complexity", async () => {
	...
	await t.step("throws error when no codec is supported", async () => {
	...
	await t.step("sets correct base configuration properties", async () => {
		const options: VideoEncoderOptions = {
			width: 800,
			height: 600,
			frameRate: 25,
	...
	await t.step("handles edge case dimensions", async () => {
		const options: VideoEncoderOptions = {
			width: 1,
			height: 1,
			frameRate: 1,
...
Deno.test("VideoConfig - upgradeEncoderConfig", async (t) => {
	...
	await t.step("configures AVC1 codec correctly", () => {
	...
	await t.step("configures HEVC codec correctly", () => {
	...
	await t.step("configures VP09 codec with bitrate adjustment", () => {
		const upgradedConfig = upgradeEncoderConfig(
			baseConfig,
			"vp09.00.10.08",
			2000000,
	...
	await t.step("configures AV01 codec with bitrate adjustment", () => {
		const upgradedConfig = upgradeEncoderConfig(
			baseConfig,
			"av01.0.08M.08",
			2000000,
	...
	await t.step("configures VP8 codec with bitrate adjustment", () => {
	...
	await t.step("handles software encoding configuration", () => {
		const upgradedConfig = upgradeEncoderConfig(baseConfig, "avc1.640028", 2000000, false);

		assertEquals(upgradedConfig.hardwareAcceleration, undefined);
	});

	await t.step("preserves base configuration properties", () => {
	...
	await t.step("handles unknown codec without specific configuration", () => {
	...
	await t.step("handles bitrate adjustments correctly for edge values", () => {
...
Deno.test("VideoConfig - Integration Tests", async (t) => {
	await t.step("complete workflow with different options", async () => {
	...
	await t.step("hardware vs software encoding selection", async () => {
		const baseOptions: VideoEncoderOptions = {
			width: 1280,
			height: 720,
			frameRate: 30,
