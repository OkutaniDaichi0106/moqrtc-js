...
Deno.test("audioEncoderConfig: throws error when no codec is supported", async () => {
    ...
    try {
        ...
        await assertRejects(async () => await audioEncoderConfig(options), Error, 'no supported audio codec');
    } finally {
        restoreMocks();
    }
});
...
Deno.test("audioEncoderConfig: handles isConfigSupported throwing error", async () => {
    setupMocks();
    try {
        mockAudioEncoder.isConfigSupported = spy(() => Promise.reject(new Error('Config check failed')));
        const options: AudioEncoderOptions = {
            sampleRate: 48000,
            channels: 2,
        };
...
Deno.test("Advanced Configuration Tests: handles malformed codec responses gracefully", async () => {
    ...
    try {
        ...
        for (const response of malformedResponses) {
            ...
            await assertRejects(async () => await audioEncoderConfig({
                sampleRate: 48000,
                channels: 2,
                preferredCodecs: ['opus']
            }), Error, 'no supported audio codec');
        }
    } finally {
        restoreMocks();
    }
